...
export function transferGet(data: transferGet) {
  return(
    ...
    before={<Avatar size={24} style={{background: '#fff'}}><Icon16Done fill="#6A9EE5" width={14} height={14}/></Avatar>}
  >
    <div>{data.data.info.firstName} {data.data.info.lastName} {declBySex(data.data.info.sex, ['передал (a)', 'передала', 'передал'])} тебе {data.sum} вакцины </div>
  </Snackbar>
  )
}
...
export function newFriend(data: newFriend) {
  return(
    ...
    before={<Avatar size={24} style={{background: '#fff'}}><Icon16Done fill="#6A9EE5" width={14} height={14}/></Avatar>}
  >
    <div>Ты получил {data.sum} вакцины за нового друга </div>
  </Snackbar>
  )
}
